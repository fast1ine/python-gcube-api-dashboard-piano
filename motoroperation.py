...
from tempmotoroperation import ContinuousMotorOperation, SingleStepMotorOperation
import time, copy

class MotorOperation(ContinuousMotorOperation, SingleStepMotorOperation):
    def __init__(self, number, robot_status, start_check, write):
        ...
        ContinuousMotorOperation.__init__(self, number, robot_status, start_check, write)
        SingleStepMotorOperation.__init__(self, number, robot_status, start_check, write)
    ...
    def run_motor_prev(self, cube_ID, speed, step_cycle=None, pause=False, discovery_group=None, option="continue") -> None:
        ...
        cube_ID_list = OperationUtils().to_list(cube_ID)
        speed_list = OperationUtils().to_list(speed)
        step_cycle_list = OperationUtils().to_list(step_cycle)
        ...
        OperationUtils().check_same_element(cube_ID_list) # 같은 원소가 있으면 error
        ...
        for i in range(len(speed_list)):
            if (str(speed_list[i]).lower() in ["stop", "sleep"]) or speed_list[i] == 0: # 스피드가 0이면 sleep 모드
                ...
                OperationUtils().float_check(speed_list[i], "stop")
        ...
        if option.lower() == "step" or option.lower() == "schedule":
            for i in range(len(step_cycle_list)):
                ...
                OperationUtils().float_check(step_cycle_list[i], "stop")
        ...
        if option.lower() == "continue": 
            ...
            for cube_ID_element in cube_ID_list:
                ...
                ### 동작
                self._write_copy(self._GenerateProtocolInstance.SetContinuousSteps_bytes(cube_ID_element, speed_list[0], discovery_group, pause))
                time.sleep(0.2)
        ### 스텝 모드
        elif option.lower() == "step": 
            for cube_ID_element in cube_ID_list:
                ...
                self._GenerateProtocolInstance._if_all_function(reg_stat, cube_ID_element, cube_ID_element == 0xFF)
                ### 동작
                self._write_copy(self._GenerateProtocolInstance.SetSingleSteps_bytes(cube_ID_element, speed_list[0], step[0], discovery_group, pause))
                time.sleep(0.2)
        ### 스케줄 모드
        elif option.lower() == "schedule": 
            for cube_ID_element in cube_ID_list:
                ...
                self._write_copy(self._GenerateProtocolInstance.SetScheduledSteps_bytes(cube_ID_element, speed_list, step, discovery_group, True))
                time.sleep(0.2)
                ### 포인트로 설정
                self._write_copy(self._GenerateProtocolInstance.SetScheduledPoints_bytes(cube_ID_element, [0], [len(speed_list)], [1], discovery_group, True))
                time.sleep(0.2)
            ### 재생
            if not pause:
                for cube_ID_element in cube_ID_list:
                    self._write_copy(self._GenerateProtocolInstance.SetPauseSteps_bytes(False, cube_ID_element, discovery_group))
                    time.sleep(0.2)
        else:
    ...
    def pause_motor(self, cube_ID=None, discovery_group=None, group_mode=False) -> None:
        ...
        def proc_op(x):
            ...
            ### 작동
            self._write_copy(self._GenerateProtocolInstance.SetPauseSteps_bytes(True, x, discovery_group, group_mode))
            time.sleep(0.2)
    ...
    def play_paused_motor(self, cube_ID=None, discovery_group=None, group_mode=False) -> None:
        ...
        def proc_op(x):
            if self._robot_status[discovery_group].controller_status.stepper_pause[x] == False: 
                ...
                ### 작동
                self._write_copy(self._GenerateProtocolInstance.SetPauseSteps_bytes(False, x, discovery_group, group_mode))
                time.sleep(0.2)
    ...
    def run_motor(self, 
        ...
        if run_option.lower() == "continue":
            self.run_motor_continue(cube_ID_list, speed_list, pause_list, discovery_group, speed_option, wait)
            return None
        elif run_option.lower() == "step":
            self.run_motor_step(cube_ID_list, speed_list, step_list, pause_list, time_list, discovery_group, speed_option, step_option, sync, time_option, wait)
            return None
        ...
        if cube_ID_list == [] or cube_ID_list == ():
            raise ValueError("cube ID must not be empty list.")
        for i in range(len(cube_ID_list)):
            cube_ID_list[i] = OperationUtils().process_cube_ID(cube_ID_list[i], connection_number)
            if cube_ID_list[i] == 0xFF and len(cube_ID_list) != 1:
                raise ValueError("If cube ID is all, input must not be length-above-2 list.")
        OperationUtils().check_same_element(cube_ID_list) # 같은 원소가 있으면 error
        ...
        if run_option.lower() != "continue":
            ...
            if time_option != None and time_option != "none":
                print("Warning. In continue mode, time_option is ignored.")
            ...
            if speed_option.upper() == "RPM":
                ...
                for i in range(len(speed_list_in)):
                    for j in range(len(speed_list_in[i])):
                        ...
                        OperationUtils().float_check(speed_list_in[i][j], "stop or sleep")
                        speed_list_in[i][j] = self._GenerateProtocolInstance.truncate_RPM_speed(speed_list_in[i][j], raise_error) # speed 자르기
                        speed_list_in[i][j] = self._GenerateProtocolInstance.RPM_to_SPS(speed_list_in[i][j]) # 단위를 RPM에서 SPS로 변경
            ### SPS 모드
                ...
                for i in range(len(speed_list_in)):
                    for j in range(len(speed_list_in[i])):
                        ...
                        OperationUtils().float_check(speed_list_in[i][j])
                        speed_list_in[i][j] = self._GenerateProtocolInstance.truncate_SPS_speed(speed_list_in[i][j], raise_error) # speed 자르기
            ### 이외 옵션 오류
            else:
        ...
        def limit_step(step_list_in, speed_list_in, sleep_list, is_schedule=False):
            ...
            if len(speed_list_in) == 1:
                speed_list_in = OperationUtils().list_product_copy(speed_list_in, run_number)
                sleep_list = OperationUtils().list_product_copy(sleep_list, run_number)
            elif len(step_list_in) == 1:
                step_list_in = OperationUtils().list_product_copy(step_list_in, run_number)
            ### CYCLE 모드
            if step_option.upper() == "CYCLE":
                ...
                for i in range(len(step_list_in)):
                    for j in range(len(step_list_in[i])):
                        ...
                        OperationUtils().float_check(step_list_in[i][j])
                        if step_list_in[i][j] < 0: 
                            step_list_in[i][j] = -step_list_in[i][j] # step이 -이면 speed를 -로 바꾸기
                            speed_list_in[i][j] = -speed_list_in[i][j]
                        step_list_in[i][j] = self._GenerateProtocolInstance.truncate_cycle_step(step_list_in[i][j], raise_error) # cycle 자르기
                        step_list_in[i][j] = self._GenerateProtocolInstance.cycle_to_step(step_list_in[i][j]) # 단위를 cycle에서 step으로 변경
            ### STEP 모드
            elif step_option.upper() == "STEP":
                ### 스텝 체크 & 변환 
                for i in range(len(step_list_in)):
                    for j in range(len(step_list_in[i])):
                        OperationUtils().integer_check(step_list_in[i][j]) # step은 integer이어야 함.
                        if step_list_in[i][j] < 0: 
                            step_list_in[i][j] = -step_list_in[i][j] # step이 -이면 speed를 -로 바꾸기
                            speed_list_in[i][j] = -speed_list_in[i][j]
                        step_list_in[i][j] = self._GenerateProtocolInstance.truncate_step_step(step_list_in[i][j], raise_error) # cycle 자르기
            ### 이외 옵션 오류
            else:
        ...
        def check_time_sync_none(is_schedule, speed_list_in, step_list_in):
            ...
            if sync and run_number > 1:
                ...
                if not is_schedule:
                    step_element_idx0 = self._GenerateProtocolInstance.step_to_cycle(step_list_in[0])
                    speed_element_idx0 = self._GenerateProtocolInstance.SPS_to_RPM(speed_list_in[0])
                    for i in range(1, len(speed_list_in)):
                        step_element_idxi = self._GenerateProtocolInstance.step_to_cycle(step_list_in[i])
                        speed_element_idxi = self._GenerateProtocolInstance.SPS_to_RPM(speed_list_in[i])
                ...
                ### 스케줄 모드 (speed: SPS, step: STEP)
                    ...
                    while j < len(speed_list_in[0]): # len(speed_list_in[0])가 계속 변함
                        ...
                        step_element_idx0j_cycle = self._GenerateProtocolInstance.step_to_cycle(step_list_in[0][j])
                        speed_element_idx0j = self._GenerateProtocolInstance.SPS_to_RPM(speed_list_in[0][j])
                        i = 1
                        while i < len(speed_list_in):
                            step_element_idxij_step = step_list_in[i][j]
                            step_element_idxij_cycle = self._GenerateProtocolInstance.step_to_cycle(step_list_in[i][j])
                            speed_element_idxij = self._GenerateProtocolInstance.SPS_to_RPM(speed_list_in[i][j])
            ...
            if not is_schedule:
                ...
                if time_option.lower() == "speed":
                    if len(speed_list_inscope) == 1:
                        speed_list_inscope = OperationUtils().list_product_copy(speed_list_inscope, run_number)
                    ...
                    if len(step_list_inscope) == 1:
                        step_list_inscope = OperationUtils().list_product_copy(step_list_inscope, run_number)
                    out_list = [[None]*len(step_list_inscope[x]) for x in range(len(step_list_inscope))]
            if len(time_list_in) == 1:
                time_list_in = OperationUtils().list_product_copy(time_list_in, run_number)
            if time_option.lower() == "speed": # speed 단위: SPS, time 단위: sec, speed_limit 이후
                for i in range(len(time_list_in)):
                    ...
                    for j in range(len(time_list_in[i])):
                        OperationUtils().float_check(time_list_in[i][j])
                        ...
                        if speed_list_inscope[i][j] == 0:
                            ...
                            speed_element = self._GenerateProtocolInstance.SPS_to_RPM(speed_list_inscope[i][j])/60 # RPS
                            out_list[i][j] = abs(round(self._GenerateProtocolInstance.cycle_to_step(speed_element*time_list_in[i][j])))
                            if out_list[i][j] > 65535:
                    ...
                    for i in range(len(step_list_inscope)):
                        for j in range(len(step_list_inscope[i])):
                            if isinstance(step_list_inscope[i][j], str) and step_list[i][j].lower() in ["stop", "sleep"]: # 스텝이 0이면 sleep 모드
                                ...
                                OperationUtils().float_check(step_list_inscope[i][j])
                                step_list_inscope[i][j] = self._GenerateProtocolInstance.truncate_cycle_step(step_list_inscope[i][j], raise_error) # cycle 자르기
                                step_list_inscope[i][j] = self._GenerateProtocolInstance.cycle_to_step(step_list_inscope[i][j]) # 단위를 cycle에서 step으로 변경
                    step_option = "STEP"
                for i in range(len(time_list_in)):
                    ### 내부 원소 길이는 이미 정리 함.
                    for j in range(len(time_list_in[i])):
                        ...
                        if step_list_inscope[i][j] == 0: # step이 0이면 sleep 모드
                            ...
                            step_element = self._GenerateProtocolInstance.step_to_cycle(step_list_inscope[i][j]) # step to cycle
                            ...
                            out_list[i][j] = round(self._GenerateProtocolInstance.RPM_to_SPS(RPM)) # RPM to SPS
                speed_option = "SPS"
            ### 겉 list 제거
            if not is_schedule:
                out_list = out_list[0]
        ...
        def convert_wait(is_schedule):
            ...
            def cal_time(speed_element, step_element):
                if speed_element == 0:
                    ...
                    speed_element = self._GenerateProtocolInstance.SPS_to_RPM(speed_element)/60 # RPS
                    step_element = self._GenerateProtocolInstance.step_to_cycle(step_element)
        ...
        ### 컨티뉴 모드
            ...
            len_check(pause_list, "continue", "pause_list")
            ### 속도 제한
