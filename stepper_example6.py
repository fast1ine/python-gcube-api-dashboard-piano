...
cube_ID = 1 # 큐브 번호 #####################################################1개짜리 안됨
motor_speed = 600 # 모터 속도
motor_cycle = 1000 # 회전 수

while not keyboard.is_pressed("q"): # q가 눌리기 전까지 쓰레드 유지
    while  PingPongThreadInstance.play_once_full_connect(): # 연결 되어있는 동안, 한 번만 실행
        PingPongThreadInstance.run_motor_sync(cube_ID, motor_speed, motor_cycle, run_option="step") # 모터 돌림 (스텝 모드)
        print(PingPongThreadInstance.get_robot_status()) # 상태 확인

cube_ID = "all"
motor_speed = 0
PingPongThreadInstance.run_motor_sync(cube_ID, motor_speed) # 모터 끔
