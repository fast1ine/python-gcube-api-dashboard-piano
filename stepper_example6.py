...
cube_ID = [1, 2] # 큐브 번호
time_list = [1, 3] # 모터 속도
motor_cycle = [1, 2] # 회전 수

while not keyboard.is_pressed("q"): # q가 눌리기 전까지 쓰레드 유지
    while PingPongThreadInstance.play_once_full_connect(): # 연결 되어있는 동안, 한 번만 실행
        PingPongThreadInstance.run_motor(cube_ID, step_list=motor_cycle, time_list=time_list, run_option="step", time_option="step", sync=True) # 모터 돌림 (스텝 모드)
