...
class ContinuousMotorOperation(MotorOperationBase):
    ...
    def run_motor_continue(self, 
        ...
        ### 디폴트 설정
        speed_list = OperationUtils().set_default(input_list=[speed_list], option_list=[speed_option], run_option="continue")
        ### speed, pause 길이 체크
        OperationUtils().len_check(speed_list, cube_ID_list, connection_number, mode_name="continue", list_name="speed_list", run_number=run_number, run_option="continue")
        OperationUtils().len_check(pause_list, cube_ID_list, connection_number, mode_name="continue", list_name="pause_list", run_number=run_number, run_option="continue")
        ### 속도 제한
...
class SingleStepsMotorOperation(MotorOperationBase):
    def __init__(self, number, robot_status, start_check, write):
        MotorOperationBase.__init__(self, number, robot_status, start_check, write)
    ...
    def run_motor_step(self, 
        ...
        ### 디폴트 설정
        speed_list, step_list = OperationUtils().set_default(input_list=[speed_list, step_list], option_list=[speed_option, step_option], run_option="step")
        ### time 옵션 분기 설정
        ### time 옵션 없음
        if time_option.lower() == "none":
            ### speed, step 길이 체크
            OperationUtils().len_check(speed_list, cube_ID_list, connection_number, mode_name="step", list_name="speed_list", run_number=run_number, run_option="step")
            OperationUtils().len_check(step_list, cube_ID_list, connection_number, mode_name="step", list_name="step_list", run_number=run_number, run_option="step")
            ### 속도, 스텝 제한
        ...
        ### time 옵션 speed 모드
            ...
            ### time, speed 길이 체크
            OperationUtils().len_check(time_list, cube_ID_list, connection_number, mode_name="step and time-speed", list_name="time_list", run_number=run_number, run_option="step")
            OperationUtils().len_check(speed_list, cube_ID_list, connection_number, mode_name="step and time-speed", list_name="speed_list", run_number=run_number, run_option="step")
            ### 시간 변환, 속도, 스텝 제한
            speed_list, sleep_list = OperationUtils().limit_speed(speed_list, speed_option, run_number, sync, run_option="step")
        ...
        ### time 옵션 step 모드
            ...
            ### time, step 길이 체크
            OperationUtils().len_check(time_list, cube_ID_list, connection_number, mode_name="step and time-step", list_name="time_list", run_number=run_number, run_option="step")
            OperationUtils().len_check(step_list, cube_ID_list, connection_number, mode_name="step and time-step", list_name="step_list", run_number=run_number, run_option="step")
            ### 시간 변환, 속도, 스텝 제한
            ...
            step_list, speed_list = OperationUtils().limit_step(step_list, speed_list, sleep_list, step_option="STEP", run_number=run_number, sync=sync, run_option="step")
        ### pause 길이 체크
        OperationUtils().len_check(pause_list, cube_ID_list, connection_number, mode_name="step", list_name="pause_list", run_number=run_number, run_option="step")
        ### 바이트 확장
...
class ScheduledStepsMotorOperation(MotorOperationBase):
    def __init__(self, number, robot_status, start_check, write):
        MotorOperationBase.__init__(self, number, robot_status, start_check, write)
    ...
    def run_motor_schedule(self, 
        ...
        ### 디폴트 체크
        OperationUtils().set_default(input_list=[speed_list, step_list, time_list], option_list=[speed_option, step_option, time_option], run_option="schedule")
        ### time 옵션 분기 설정
        ### time 옵션 없음
        if time_option.lower() == "none":
            ### speed, step 리스트가 list of list인지 체크 (dataframe, array 이용? (numpy, pandas 등 사용))
            OperationUtils().check_list_of_list(speed_list, mode_name="schedule", list_name="speed_list", run_option="schedule")
            OperationUtils().check_list_of_list(step_list, mode_name="schedule", list_name="step_list", run_option="schedule")
            ### speed, step 리스트 길이 체크
            OperationUtils().len_check(speed_list, cube_ID_list, connection_number, mode_name="schedule", list_name="speed_list", run_number=run_number, run_option="schedule")
            OperationUtils().len_check(step_list, cube_ID_list, connection_number, mode_name="schedule", list_name="step_list", run_number=run_number, run_option="schedule")
            ### 내부 원소 길이 체크
            OperationUtils().len_check_elemental_list(speed_list, step_list, mode_name="schedule", input_list1_name="speed_list", input_list2_name="step_list")
            ### sync 모드일 때 내부 원소 길이 체크
            OperationUtils().len_check_elemental_list_in_sync(sync, speed_list, step_list, mode_name="schedule sync", input_list1_name="speed_list", input_list2_name="step_list")
            ### 속도, 스텝 제한
            speed_list, sleep_list = OperationUtils().limit_speed(speed_list, speed_option, run_number, sync, run_option="schedule")
            step_list, speed_list = OperationUtils().limit_step(step_list, speed_list, sleep_list, step_option, run_number, sync, run_option="schedule")
        ### time 옵션 speed 모드
        elif time_option.lower() == "speed":
            ### time, speed 리스트가 list of list인지 체크 (dataframe, array 이용? (numpy, pandas 등 사용))
            OperationUtils().check_list_of_list(time_list, mode_name="schedule", list_name="time_list", run_option="schedule")
            OperationUtils().check_list_of_list(speed_list, mode_name="schedule", list_name="speed_list", run_option="schedule")
            ### time, speed 리스트 길이 체크
            OperationUtils().len_check(time_list, cube_ID_list, connection_number, mode_name="schedule", list_name="time_list", run_number=run_number, run_option="schedule")
            OperationUtils().len_check(speed_list, cube_ID_list, connection_number, mode_name="schedule", list_name="speed_list", run_number=run_number, run_option="schedule")
            ### 내부 원소 길이 체크
            OperationUtils().len_check_elemental_list(time_list, speed_list, mode_name="schedule", input_list1_name="time_list", input_list2_name="speed_list")
            ### sync 모드일 때 내부 원소 길이 체크
            OperationUtils().len_check_elemental_list_in_sync(sync, time_list, speed_list, mode_name="schedule sync", input_list1_name="time_list", input_list2_name="speed_list")
            ### 시간 변환, 속도, 스텝 제한
            speed_list, sleep_list = OperationUtils().limit_speed(speed_list, speed_option, run_number, sync, run_option="schedule")
            step_list, _, speed_option, step_option = OperationUtils().convert_time_list(time_list, speed_list, step_list=None, speed_option=speed_option, step_option=None, run_number=run_number, sync=sync, run_option="schedule", time_option="speed")
            step_list, speed_list = OperationUtils().limit_step(step_list, speed_list, sleep_list, step_option="STEP", run_number=run_number, sync=sync, run_option="schedule")
        ### time 옵션 step 모드
        elif time_option.lower() == "step":
            ...
            OperationUtils().len_check(step_list, cube_ID_list, connection_number, mode_name="schedule", list_name="step_list", run_number=run_number, run_option="schedule")
            ### 내부 원소 길이 체크
            OperationUtils().len_check_elemental_list(time_list, step_list, mode_name="schedule", input_list1_name="time_list", input_list2_name="step_list")
            ### sync 모드일 때 내부 원소 길이 체크
            OperationUtils().len_check_elemental_list_in_sync(sync, time_list, step_list, mode_name="schedule sync", input_list1_name="time_list", input_list2_name="step_list")
            ### 시간 변환, 속도, 스텝 제한
            speed_list, step_list, speed_option, step_option = OperationUtils().convert_time_list(time_list, speed_list=None, step_list=step_list, speed_option=None, step_option=step_option, run_number=run_number, sync=sync, run_option="schedule", time_option="step")
            speed_list, sleep_list = OperationUtils().limit_speed(speed_list, speed_option, run_number, sync, run_option="schedule")
            step_list, speed_list = OperationUtils().limit_step(step_list, speed_list, sleep_list, step_option="STEP", run_number=run_number, sync=sync, run_option="schedule")
        ### pause 길이 체크
        OperationUtils().len_check(pause_list, cube_ID_list, connection_number, mode_name="schedule", list_name="pause_list", run_number=run_number, run_option="schedule")
        ### 바이트 확장
        cube_ID_list, speed_list, step_list, pause_list = OperationUtils().expand_bytes(cube_ID_list, connection_number, run_number, speed_list, step_list, pause_list)
        ### sync 모드 처리
        speed_list, step_list = OperationUtils().check_time_sync_none(speed_list, step_list, sync, run_number, run_option="schedule")
        ### wait 변환 (wait = "step" 또는 "schedule"이면 현재 run_option에 맞춰서 알아서 계산)
        if isinstance(wait, str):
            wait = OperationUtils().convert_wait(speed_list, step_list, pause_list, run_option="step")
        ### 작동 처리 (discovery_group 처리 해야함)
...
class ScheduledPointsMotorOperation(MotorOperationBase):
    def __init__(self, number, robot_status, start_check, write):
        MotorOperationBase.__init__(self, number, robot_status, start_check, write)
    ...
    def play_motor_schedule(self, 
        ...
        start_point_list = OperationUtils().to_list(start_point_list)
        stop_point_list = OperationUtils().to_list(stop_point_list)
        repeat_list = OperationUtils().to_list(repeat_list)
        cube_ID_list = OperationUtils().to_list(cube_ID_list)
        pause_list = OperationUtils().to_list(pause_list)
        start_and_stop_list = OperationUtils().to_list(start_and_stop_list)
        ### 큐브 ID 리스트 처리
        cube_ID_list = OperationUtils().process_cube_ID_list(cube_ID_list, connection_number)
        ### run_number 정의
        run_number = OperationUtils().set_run_number(cube_ID_list, connection_number)
        ...
        ### list of list 체크
        OperationUtils().check_list_of_list(start_point_list, mode_name="point", list_name="start_point_list", run_option="point")
        OperationUtils().check_list_of_list(stop_point_list, mode_name="point", list_name="stop_point_list", run_option="point")
        OperationUtils().check_list_of_list(repeat_list, mode_name="point", list_name="repeat_list", run_option="point")
        ### list 길이 체크
        OperationUtils().len_check(start_point_list, cube_ID_list, connection_number, mode_name="point", list_name="start_point_list", run_number=run_number, run_option="point")
        OperationUtils().len_check(stop_point_list, cube_ID_list, connection_number, mode_name="point", list_name="stop_point_list", run_number=run_number, run_option="point")
        OperationUtils().len_check(repeat_list, cube_ID_list, connection_number, mode_name="point", list_name="repeat_list", run_number=run_number, run_option="point")
        OperationUtils().len_check(pause_list, cube_ID_list, connection_number, mode_name="point", list_name="pause_list", run_number=run_number, run_option="point")
        ### 리스트 확장
        ...
        if len(start_point_list) == 1: # 길이가 1이면 run_number 개수만큼 늘림
            start_point_list = OperationUtils().list_product_copy(start_point_list, run_number)
        if len(stop_point_list) == 1: # 길이가 1이면 run_number 개수만큼 늘림
            stop_point_list = OperationUtils().list_product_copy(stop_point_list, run_number)
        if len(repeat_list) == 1: # 길이가 1이면 run_number 개수만큼 늘림
            repeat_list = OperationUtils().list_product_copy(repeat_list, run_number)
        ...
        for i, cube_ID_element in enumerate(cube_ID_list):
            for j in range(len(start_point_list[i])):
                ...
                OperationUtils().integer_check(start_point_list[i][j])
                OperationUtils().integer_check(stop_point_list[i][j])
                OperationUtils().integer_check(repeat_list[i][j])
        ...
        ### wait 처리
        if isinstance(wait, str):
            wait = OperationUtils().convert_wait_point(cube_ID_list, self._robot_status, discovery_group, start_point_list, stop_point_list, repeat_list, pause_list)
        ### 작동 처리 (discovery_group 처리 해야함)
        sending_bytes = b""
        for i, cube_ID_element in enumerate(cube_ID_list):
            ### status 등록
