class CubeOperationUtils():
    ...
    ### 메소드 체크
    def check_method(self, method, cube_ID, discovery_group, get_robot_status, set_robot_status):
        if not isinstance(method, str):
            ...
            connection_number = get_robot_status(discovery_group, "controller_status", "connection_number")
            if method.lower() == "periodic":
                if cube_ID == 0xFF:
                    for i in range(connection_number):
                        set_robot_status(discovery_group, "controller_status", "get_sensor_mode[{}]".format(i), "\"periodic\"")
                else:
                    set_robot_status(discovery_group, "controller_status", "get_sensor_mode[{}]".format(cube_ID), "\"periodic\"")
            elif method.lower() == "oneshot": 
                if cube_ID == 0xFF:
                    prev_mode = get_robot_status(discovery_group, "controller_status", "get_sensor_mode")
                    periodic_flag = False
                    for i in range(connection_number):
                        if prev_mode[i] == "periodic":
                            periodic_flag = True
                        set_robot_status(discovery_group, "controller_status", "get_sensor_mode[{}]".format(i), "\"oneshot\"")
                    if periodic_flag:
                        print("Warning. All periodic get_sensor in group ID will be stop.")
                else:
                    prev_mode = get_robot_status(discovery_group, "controller_status", "get_sensor_mode[{}]".format(cube_ID))
                    if prev_mode == "periodic":
                        raise ValueError("Cannot set get_sensor mode as \"oneshot\", if the current get_sensor mode is \"periodic\".")
                    else:
                        set_robot_status(discovery_group, "controller_status", "get_sensor_mode[{}]".format(cube_ID), "\"oneshot\"")
