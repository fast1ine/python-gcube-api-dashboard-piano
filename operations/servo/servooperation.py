...
class ServoOperation():
    ...
    def run_servo_schedule(self, 
        ...
        for i, cube_ID_element in enumerate(cube_ID_list):
            ...
            ### servo status 등록
            self._robot_status[discovery_group].controller_status.servo_mode[cube_ID_element] = "point"
            self._robot_status[discovery_group].controller_status.servo_angle_schedule[cube_ID_element] = servo_angle_list[i]
            self._robot_status[discovery_group].controller_status.servo_timeout_schedule[cube_ID_element] = servo_timeout_list[i]
            ### bytes 붙이기
        ...
        ServoOperationUtils().wait_until_agg_set(self._get_robot_status, self._set_robot_status, discovery_group, connection_number)
        ### 포인트로 작동
        time.sleep(0.2)
        sending_bytes = b""
        for i, cube_ID_element in enumerate(cube_ID_list):
            sending_bytes += self._GenerateProtocolInstance.SetScheduledPoints_bytes(cube_ID_element, [0], [len(speed_list[i])-1], [1], discovery_group, 
    ...
    def set_servo_schedule(self, cube_ID_list="all", servo_angle_list=None, servo_duration=None, pause_list=True, speed_list=None, step_list=None, 
        time_list=None, discovery_group=None, speed_option="RPM", step_option="CYCLE", sync=False, time_option=None, wait=0) -> None:
        self.run_servo_schedule(cube_ID_list=cube_ID_list, servo_angle_list=servo_angle_list, servo_duration=servo_duration, pause_list=pause_list, 
            speed_list=speed_list, step_list=step_list, time_list=time_list, discovery_group=discovery_group, speed_option=speed_option, 
    ...
    def play_servo_schedule(self, 
        ...
        ### wait 처리
        if isinstance(wait, str):
            wait = ServoOperationUtils().convert_wait_point(cube_ID_list, self._robot_status, discovery_group, start_point_list, stop_point_list, repeat_list, pause_list)
        ### 작동 처리 (discovery_group 처리 해야함)
        sending_bytes = b""
        for i, cube_ID_element in enumerate(cube_ID_list):
            ### stepper status 등록
            ...
            sending_bytes += self._GenerateProtocolInstance.SetScheduledPoints_bytes(cube_ID_element, start_point_list[i], stop_point_list[i], 
                repeat_list[i], discovery_group, pause_list[i], step_type=4)
        ### 바이트 쓰기, 작동
        if connection_number > 1:
            sending_bytes = self._GenerateProtocolInstance.SetAggregateSteps_bytes(discovery_group, sending_bytes)
        self._write_copy(sending_bytes)
